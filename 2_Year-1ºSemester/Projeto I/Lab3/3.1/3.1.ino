...
void loop() {
  ...
  switch (system_state) {
    case RED:
      ...
      // <CODE>
      // CONDITIONS FOR STATE TRANSITION - different state transitions should be included as different ifs 
      if (buttonPressed()) {  //change state when button is pressed
          // CODE TO EXECUTE ON THE STATE TRANSITION: executed only on the transition
